...
if __name__ == '__main__':
    ...
    system_info()
    print('--- Params ---')
    print('rapids_mg basic')
    print(f'{DATA_PATH=}')

    # Initialize multi-GPU Dask cluster (4x H100)
    ...
    # Lazy loader: read_elem_lazy (anndata >=0.12) or read_elem_as_dask
    ...
    timers = MemoryTimer(
        ...
        csv_columns={'library': 'rapids_mg', 'test': 'basic',
                     'dataset': DATA_NAME})
    ...
    with timers('Load data'):
        ...
        data = ad.AnnData(
            X=read_dask(f['X'], chunks=(CHUNK_SIZE, shape[1])),
    ...
    with timers('Feature selection'):
        ...
        # Rechunk evenly across workers after subsetting to HVGs
    ...
    with timers('PCA'):
        ...
        # Materialize PCA for downstream non-Dask operations
    ...
    with timers('Plot embedding'):
        ...
        plt.savefig(
            f'sc-benchmarking/figures/rapids_mg_embedding_{DATA_NAME}.png',
            bbox_inches='tight', dpi=300)

    # Markers on CPU (logreg not available in multi-GPU mode)
